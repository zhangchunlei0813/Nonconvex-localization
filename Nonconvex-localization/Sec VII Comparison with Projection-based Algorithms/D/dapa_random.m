...
for l=1:L
    ...
    for j=1:kk
        ...
        if T(1)==1&&T(5)==1
            ...
            a15=1/(max(N(1),N(5))+1);
            a1=1-a12-a15;
            v1(:,j)=a1*x1(:,j)+a12*x2(:,j)+a15*x5(:,j);
        ...
        if T(1)==1&&T(2)==1&&T(6)==1
            a21=1/(max(N(1),N(2))+1);
            a23=1/(max(N(2),N(3))+1);
            a24=1/(max(N(2),N(4))+1);
            a2=1-a21-a23-a24;
            v2(:,j)=a2*x2(:,j)+a21*x1(:,j)+a23*x3(:,j)+a24*x4(:,j);
        ...
        if T(1)==1&&T(2)==1&&T(6)==0
            a21=1/(max(N(1),N(2))+1);
            a23=1/(max(N(2),N(3))+1);
            a2=1-a21-a23;
            v2(:,j)=a2*x2(:,j)+a21*x1(:,j)+a23*x3(:,j);
            w2(:,j)=v2(:,j)-1/(j+2)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2)-delta*nf));
            x2(:,j+1)=w2(:,j)-1/(j+1)*(w2(:,j)-proj(w2(:,j),a(:,2),r(2)+delta*nf));
            %        x2(:,j+1)=v2(:,j)-1/(j+1)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2+9*(l-1))+delta*nf));
        end
        if T(1)==1&&T(2)==0&&T(6)==1
            a21=1/(max(N(1),N(2))+1);
            a24=1/(max(N(2),N(4))+1);
            a2=1-a21-a24;
            v2(:,j)=a2*x2(:,j)+a21*x1(:,j)+a24*x4(:,j);
            w2(:,j)=v2(:,j)-1/(j+2)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2)-delta*nf));
            x2(:,j+1)=w2(:,j)-1/(j+1)*(w2(:,j)-proj(w2(:,j),a(:,2),r(2)+delta*nf));
            %        x2(:,j+1)=v2(:,j)-1/(j+1)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2+9*(l-1))+delta*nf));
        end
        if T(1)==0&&T(2)==1&&T(6)==1
            a23=1/(max(N(2),N(3))+1);
            a24=1/(max(N(2),N(4))+1);
            a2=1-a23-a24;
            v2(:,j)=a2*x2(:,j)+a23*x3(:,j)+a24*x4(:,j);
            w2(:,j)=v2(:,j)-1/(j+2)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2)-delta*nf));
            x2(:,j+1)=w2(:,j)-1/(j+1)*(w2(:,j)-proj(w2(:,j),a(:,2),r(2)+delta*nf));
            %        x2(:,j+1)=v2(:,j)-1/(j+1)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2+9*(l-1))+delta*nf));
        end
        if T(1)==1&&T(2)==0&&T(6)==0
            a21=1/(max(N(1),N(2))+1);
            a2=1-a21;
            v2(:,j)=a2*x2(:,j)+a21*x1(:,j);
            w2(:,j)=v2(:,j)-1/(j+2)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2)-delta*nf));
            x2(:,j+1)=w2(:,j)-1/(j+1)*(w2(:,j)-proj(w2(:,j),a(:,2),r(2)+delta*nf));
            %        x2(:,j+1)=v2(:,j)-1/(j+1)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2+9*(l-1))+delta*nf));
        end
        if T(1)==0&&T(2)==1&&T(6)==0
            a23=1/(max(N(2),N(3))+1);
            a2=1-a23;
            v2(:,j)=a2*x2(:,j)+a23*x3(:,j);
            w2(:,j)=v2(:,j)-1/(j+2)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2)-delta*nf));
            x2(:,j+1)=w2(:,j)-1/(j+1)*(w2(:,j)-proj(w2(:,j),a(:,2),r(2)+delta*nf));
            %        x2(:,j+1)=v2(:,j)-1/(j+1)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2+9*(l-1))+delta*nf));
        end
        if T(1)==0&&T(2)==0&&T(6)==1
            a24=1/(max(N(2),N(4))+1);
            a2=1-a24;
            v2(:,j)=a2*x2(:,j)+a24*x4(:,j);
            w2(:,j)=v2(:,j)-1/(j+2)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2)-delta*nf));
            x2(:,j+1)=w2(:,j)-1/(j+1)*(w2(:,j)-proj(w2(:,j),a(:,2),r(2)+delta*nf));
            %        x2(:,j+1)=v2(:,j)-1/(j+1)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2+9*(l-1))+delta*nf));
        end
        if T(1)==0&&T(2)==0&&T(6)==0
            a2=1;
            v2(:,j)=a2*x2(:,j);
            w2(:,j)=v2(:,j)-1/(j+2)*(v2(:,j)-proj(v2(:,j),a(:,2),r(2)-delta*nf));
            x2(:,j+1)=w2(:,j)-1/(j+1)*(w2(:,j)-proj(w2(:,j),a(:,2),r(2)+delta*nf));
        ...
        if T(2)==1&&T(3)==1
            ...
            a34=1/(max(N(4),N(3))+1);
            a3=1-a32-a34;
            v3(:,j)=a3*x3(:,j)+a32*x2(:,j)+a34*x4(:,j);
        ...
        if T(3)==1&&T(4)==1&&T(6)==1;
            a43=1/(max(N(3),N(4))+1);
            a45=1/(max(N(4),N(5))+1);
            a42=1/(max(N(4),N(2))+1);
            a4=1-a43-a45-a42;
            v4(:,j)=a4*x4(:,j)+a43*x3(:,j)+a45*x5(:,j)+a42*x2(:,j);
        ...
        if T(3)==1&&T(4)==1&&T(6)==0;
            a43=1/(max(N(3),N(4))+1);
            a45=1/(max(N(4),N(5))+1);
            a4=1-a43-a45;
            v4(:,j)=a4*x4(:,j)+a43*x3(:,j)+a45*x5(:,j);
            w4(:,j)=v4(:,j)-1/(j+2)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4)-delta*nf));
            x4(:,j+1)=w4(:,j)-1/(j+1)*(w4(:,j)-proj(w4(:,j),a(:,4),r(4)+delta*nf));
            %  x4(:,j+1)=v4(:,j)-1/(j+1)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4+9*(l-1))+delta*nf));
        end
        if T(3)==1&&T(4)==0&&T(6)==1;
            a43=1/(max(N(3),N(4))+1);
            a42=1/(max(N(4),N(2))+1);
            a4=1-a43-a42;
            v4(:,j)=a4*x4(:,j)+a43*x3(:,j)+a42*x2(:,j);
            w4(:,j)=v4(:,j)-1/(j+2)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4)-delta*nf));
            x4(:,j+1)=w4(:,j)-1/(j+1)*(w4(:,j)-proj(w4(:,j),a(:,4),r(4)+delta*nf));
            %  x4(:,j+1)=v4(:,j)-1/(j+1)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4+9*(l-1))+delta*nf));
        end
        if T(3)==0&&T(4)==1&&T(6)==1;
            a45=1/(max(N(4),N(5))+1);
            a42=1/(max(N(4),N(2))+1);
            a4=1-a45-a42;
            v4(:,j)=a4*x4(:,j)+a45*x5(:,j)+a42*x2(:,j);
            w4(:,j)=v4(:,j)-1/(j+2)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4)-delta*nf));
            x4(:,j+1)=w4(:,j)-1/(j+1)*(w4(:,j)-proj(w4(:,j),a(:,4),r(4)+delta*nf));
            %  x4(:,j+1)=v4(:,j)-1/(j+1)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4+9*(l-1))+delta*nf));
        end
        if T(3)==1&&T(4)==0&&T(6)==0
            a43=1/(max(N(3),N(4))+1);
            a4=1-a43;
            v4(:,j)=a4*x4(:,j)+a43*x3(:,j);
            w4(:,j)=v4(:,j)-1/(j+2)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4)-delta*nf));
            x4(:,j+1)=w4(:,j)-1/(j+1)*(w4(:,j)-proj(w4(:,j),a(:,4),r(4)+delta*nf));
            %  x4(:,j+1)=v4(:,j)-1/(j+1)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4+9*(l-1))+delta*nf));
        end
        if T(3)==0&&T(4)==1&&T(6)==0
            a45=1/(max(N(4),N(5))+1);
            a4=1-a45;
            v4(:,j)=a4*x4(:,j)+a45*x5(:,j);
            w4(:,j)=v4(:,j)-1/(j+2)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4)-delta*nf));
            x4(:,j+1)=w4(:,j)-1/(j+1)*(w4(:,j)-proj(w4(:,j),a(:,4),r(4)+delta*nf));
            %  x4(:,j+1)=v4(:,j)-1/(j+1)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4+9*(l-1))+delta*nf));
        end
        if T(3)==0&&T(4)==0&&T(6)==1;
            a42=1/(max(N(4),N(2))+1);
            a4=1-a42;
            v4(:,j)=a4*x4(:,j)+a42*x2(:,j);
            w4(:,j)=v4(:,j)-1/(j+2)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4)-delta*nf));
            x4(:,j+1)=w4(:,j)-1/(j+1)*(w4(:,j)-proj(w4(:,j),a(:,4),r(4)+delta*nf));
            %  x4(:,j+1)=v4(:,j)-1/(j+1)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4+9*(l-1))+delta*nf));
        end
        if T(3)==0&&T(4)==0&&T(6)==0
            a4=1;
            v4(:,j)=a4*x4(:,j);
            w4(:,j)=v4(:,j)-1/(j+2)*(v4(:,j)-proj(v4(:,j),a(:,4),r(4)-delta*nf));
            x4(:,j+1)=w4(:,j)-1/(j+1)*(w4(:,j)-proj(w4(:,j),a(:,4),r(4)+delta*nf));
        ...
        if T(5)==1&&T(4)==1
            ...
            a51=1/(max(N(1),N(5))+1);
            a5=1-a54-a51;
            v5(:,j)=a5*x5(:,j)+a54*x4(:,j)+a51*x1(:,j);
